...
export function VistaRutaDiaria({ todayCollections = [], onRegisterPayment, onReorderClients }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-[#1E1E1E] p-5 rounded-3xl border border-[#E6DCD2] dark:border-[#332F2C] warm-shadow flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition-colors duration-300">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          <div className="flex-1 md:flex-initial bg-[#FAF8F5] dark:bg-[#24211E] p-3 rounded-2xl border border-[#E6DCD2] dark:border-[#332F2C] text-center min-w-[100px] transition-colors duration-300">
            <span className="text-[10px] font-bold text-[#6E615A] dark:text-[#E5E7EB] block uppercase">Recaudado Hoy</span>
            <strong className="text-sm font-extrabold text-[#2D7A5D] dark:text-[#3D9970]">
              {formatCurrency(totalCollectedToday)}
            </strong>
          </div>

          <div className="flex-1 md:flex-initial bg-[#FAF8F5] dark:bg-[#24211E] p-3 rounded-2xl border border-[#E6DCD2] dark:border-[#332F2C] text-center min-w-[100px] transition-colors duration-300">
            <span className="text-[10px] font-bold text-[#6E615A] dark:text-[#E5E7EB] block uppercase">Meta Diaria</span>
            <strong className="text-sm font-extrabold text-[#2C221E] dark:text-[#F3F4F6]">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <div className="flex items-center gap-1.5 bg-white dark:bg-[#1E1E1E] p-1 rounded-2xl border border-[#E6DCD2] dark:border-[#332F2C] warm-shadow transition-colors duration-300">
          <button
            ...
            className={`flex-1 sm:flex-initial px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all ${
              filter === 'PENDING'
                ...
                : 'text-[#6E615A] dark:text-[#E5E7EB] hover:bg-[#FAF8F5] dark:hover:bg-[#24211E]'
            }`}
          >
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            placeholder="Buscar por cliente, alias..."
            className="w-full pl-9 pr-3 py-2 bg-white dark:bg-[#1E1E1E] border border-[#E6DCD2] dark:border-[#332F2C] rounded-2xl text-xs font-semibold text-[#2C221E] dark:text-[#F3F4F6] placeholder-[#9E918A] dark:placeholder-[#6E615A] focus:outline-none focus:border-[#D96B27] warm-shadow transition-colors duration-300"
          />
        </div>
      </div>
      ...
      {filteredCollections.length === 0 ? (
        <div className="bg-white dark:bg-[#1E1E1E] rounded-3xl p-12 text-center border border-[#E6DCD2] dark:border-[#332F2C] warm-shadow transition-colors duration-300">
          <div className="w-12 h-12 mx-auto rounded-full bg-[#EEF6F2] dark:bg-[#1E2D27] text-[#2D7A5D] dark:text-[#3D9970] flex items-center justify-center mb-3">
            <CheckCircle2 className="w-6 h-6" />
          </div>
          <h3 className="text-base font-extrabold text-[#2C221E] dark:text-[#F3F4F6]">
            ¡No hay cobros pendientes!
          </h3>
          <p className="text-xs text-[#6E615A] dark:text-[#E5E7EB] max-w-sm mx-auto mt-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredCollections.map(({ loan, isPaidToday, amountPaidToday }, idx) => {
            ...
            return (
              <div
                key={loan.id}
                className={`bg-white dark:bg-[#1E1E1E] rounded-3xl p-5 border transition-all warm-shadow flex flex-col justify-between ${
                  isPaidToday
                    ? 'border-[#2D7A5D]/30 dark:border-[#3D9970]/30 bg-[#EEF6F2]/30 dark:bg-[#1E2D27]/30'
                    : isOverdue
                    ? 'border-[#C84B31]/30 dark:border-[#E57373]/30'
                    : 'border-[#E6DCD2] dark:border-[#332F2C]'
                }`}
              >
                <div>
                  <div className="flex justify-between items-start gap-2 border-b border-[#E6DCD2]/60 dark:border-[#332F2C] pb-3 mb-3">
                    <div className="flex items-start gap-2">
                      <span className="text-[11px] font-black text-white bg-[#2C221E] dark:bg-[#332F2C] px-2 py-0.5 rounded-lg shrink-0">
                        #{idx + 1}
                      </span>
                      <div>
                        <h3 className="font-extrabold text-sm text-[#2C221E] dark:text-[#F3F4F6] flex items-center gap-1.5 flex-wrap">
                          <span>{loan.clientName}</span>
                          {loan.clientAlias && (
                            <span className="text-[10px] font-extrabold bg-[#FDF3ED] dark:bg-[#2C221E] text-[#D96B27] dark:text-[#E07A5F] px-2 py-0.5 rounded-full border border-[#D96B27]/30">
                              ({loan.clientAlias})
                            </span>
                          )}
                        </h3>
                    ...
                    <div className="flex flex-col items-end gap-1">
                      ...
                      {onReorderClients && (
                        <div className="flex items-center gap-0.5 bg-[#FAF8F5] dark:bg-[#24211E] p-0.5 rounded-lg border border-[#E6DCD2] dark:border-[#332F2C]">
                          <button
                            onClick={() => handleMoveOrder(idx, 'UP')}
                            disabled={idx === 0}
                            className="p-1 rounded-md hover:bg-white dark:hover:bg-[#1E1E1E] text-[#6E615A] dark:text-[#E5E7EB] hover:text-[#D96B27] disabled:opacity-30 disabled:hover:bg-transparent cursor-pointer"
