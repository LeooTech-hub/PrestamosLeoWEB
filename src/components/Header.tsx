      ...
      <header className="sticky top-0 z-30 h-auto py-2.5 px-4 w-full bg-[#FAF8F5]/95 dark:bg-[#1C1917]/95 backdrop-blur-md border-b border-[#E6DCD2] dark:border-[#3D352E] flex items-center justify-center transition-colors duration-300 overflow-hidden">
        <div className="w-full max-w-6xl mx-auto flex items-center justify-between gap-2 flex-nowrap">
          ...
          <div className="flex flex-col sm:flex-row items-end sm:items-center gap-1 shrink-0">
            ...
            {/* FILA 1 (Arriba en móvil, integrada en desktop) */}
            <div className="flex items-center gap-1">
              {/* + Crear Préstamo — SOLO visible en sm+ */}
              <button
                onClick={onOpenQuickCreateLoan}
                className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg terracotta-gradient text-white text-xs font-bold shadow-sm hover:brightness-110 active:scale-95 transition-all whitespace-nowrap mr-1"
              >
              ...
              <button
                ...
                className="w-8 h-8 p-1.5 flex items-center justify-center rounded-lg bg-transparent border-none text-[#6E615A] dark:text-[#C2B29F] hover:text-[#D96B27] dark:hover:text-[#E07A5F] hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-all active:scale-95"
              >
                <RefreshCw className="w-5 h-5 sm:w-[18px] sm:h-[18px] text-[#2C221E] dark:text-[#EAE0D5]" />
              </button>

              {/* Campana de Alertas */}
              <button
                onClick={() => setIsNotificationsOpen(!isNotificationsOpen)}
                className="relative w-8 h-8 p-1.5 flex items-center justify-center rounded-lg bg-transparent border-none text-[#6E615A] dark:text-[#C2B29F] hover:text-[#D96B27] dark:hover:text-[#E07A5F] hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-all active:scale-95"
                title="Centro de Alertas"
              >
                <Bell className="w-5 h-5 sm:w-[18px] sm:h-[18px] text-[#2C221E] dark:text-[#EAE0D5]" />
                {alertsCount > 0 && (
            ...
            {/* FILA 2 (Abajo en móvil, tamaño compacto) */}
            <div className="flex items-center gap-1">
              {/* Modo Día / Oscuro */}
              <button
                onClick={toggleTheme}
                title={theme === 'dark' ? 'Cambiar a Modo Día' : 'Activar Modo Nocturno Cálido'}
                className="p-1 flex items-center justify-center rounded-lg bg-transparent border-none text-[#6E615A] dark:text-[#C2B29F] hover:text-[#E89D4F] dark:hover:text-[#E89D4F] hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-all active:scale-95 opacity-80 hover:opacity-100"
              >
                {theme === 'dark' ? (
                  <Sun className="w-[18px] h-[18px] sm:w-[18px] sm:h-[18px] text-[#E89D4F]" />
                ) : (
                  <Moon className="w-[18px] h-[18px] sm:w-[18px] sm:h-[18px] text-[#2C221E]" />
                )}
              </button>
              ...
              <button
                ...
                title="Cerrar Sesión"
                className="p-1 flex items-center justify-center gap-1 rounded-lg bg-transparent border-none text-[#6E615A] dark:text-[#C2B29F] hover:text-[#C84B31] dark:hover:text-[#C84B31] hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-all active:scale-95 opacity-80 hover:opacity-100"
              >
                <LogOut className="w-[18px] h-[18px] sm:w-[18px] sm:h-[18px]" />
