...
export function UserManagementModal({ isOpen, onClose }) {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', role: 'COBRADOR' });
  ...
  const handleDelete = async (userItem) => {
    ...
    if (!confirm(`¿Eliminar al usuario "${userItem.name}"? Esta acción no se puede deshacer.`)) return;
    setError('');
    setSuccess('');
    try {
      await api.delete(`/users/${userItem.id}`);
      setSuccess(`Usuario "${userItem.name}" eliminado.`);
      await loadUsers();
    } catch (err) {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E6DCD2] dark:border-[#3D352E] bg-white dark:bg-[#2A241F] shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#6E615A] dark:text-[#C2B29F]">Control de acceso — Registro de Cobradores</p>
            </div>
          </div>
          <button
            ...
            <form onSubmit={handleCreate} className="bg-white dark:bg-[#2A241F] rounded-2xl border border-[#E6DCD2] dark:border-[#3D352E] p-4 space-y-3">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-xl border border-[#E6DCD2] dark:border-[#3D352E] bg-[#FAF8F5] dark:bg-[#1C1917] text-[#2C221E] dark:text-[#EAE0D5] text-sm placeholder-[#B5A49A] focus:outline-none focus:ring-2 focus:ring-[#D96B27]/50 transition-all"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-3 py-2 pr-10 rounded-xl border border-[#E6DCD2] dark:border-[#3D352E] bg-[#FAF8F5] dark:bg-[#1C1917] text-[#2C221E] dark:text-[#EAE0D5] text-sm placeholder-[#B5A49A] focus:outline-none focus:ring-2 focus:ring-[#D96B27]/50 transition-all"
                  />
                  <button
                    type="button"
                    onClick={() => setShowPassword(v => !v)}
                    className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#6E615A] hover:text-[#D96B27] transition-colors"
                  >
              ...
              <div className="flex gap-2 pt-1">
                <button
                  ...
                  className="flex-1 py-2 rounded-xl border border-[#E6DCD2] dark:border-[#3D352E] text-[#6E615A] dark:text-[#C2B29F] text-sm font-semibold hover:bg-[#F4EBE1] transition-all"
                >
                  Cancelar
                </button>
                <button
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={loadUsers}
                disabled={loading}
                className="w-7 h-7 flex items-center justify-center rounded-lg text-[#6E615A] hover:bg-[#F4EBE1] transition-all disabled:opacity-50"
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {u.id !== currentUser?.id && (
                        <button
                          ...
                          className="w-7 h-7 flex items-center justify-center rounded-lg text-[#C84B31] hover:bg-[#FDF2F0] transition-all active:scale-95"
