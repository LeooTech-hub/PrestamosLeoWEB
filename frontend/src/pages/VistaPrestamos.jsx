...
export function VistaPrestamos({
  loans = [],
  onRegisterPayment,
  onUpdateLoan,
  onDeleteLoan,
  ...
  // Obtener apodo soportando posibles nombres usados por backend/frontend
  const getLoanNickname = (loan) => {
    return (
      loan?.clientNickname ||
      loan?.client_nickname ||
      loan?.nickname ||
      loan?.apodo ||
      loan?.clientAlias ||
      loan?.client_alias ||
  ...
  const filteredLoans = (loans || []).filter((loan) => {
    ...
    if (filter === 'ACTIVE' && loan?.status !== 'ACTIVE') {
      return false;
    }

    if (filter === 'OVERDUE' && loan?.status !== 'OVERDUE') {
      return false;
    }

    if (filter === 'PAID' && loan?.status !== 'PAID') {
      return false;
    }
    ...
    // Si no hay búsqueda, mostrar según filtro normal
    if (!term) return true;
    ...
    const clientNickname = String(
      getLoanNickname(loan)
    ).toLowerCase();
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            placeholder="Buscar por cliente, apodo o teléfono..."
                    ...
                    <div className="min-w-0">
                      ...
                      {nickname && (
                        <div className="mt-1 mb-1">
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-[#FDF3ED] text-[#D96B27] border border-[#D96B27]/25">
                            ({nickname})
                          </span>
                        </div>
                      )}
                      ...
                      <div className="flex items-center gap-1 text-xs text-[#6E615A] mt-0.5">
                        ...
                        <span>
                          {loan.clientPhone || loan.client_phone}
                        </span>
